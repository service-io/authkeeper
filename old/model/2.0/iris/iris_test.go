...
func TestSelectEvaluator_Eval(t *testing.T) {
	...
	nc := WithColumn[string]("name")
	ac := WithColumn[string]("age")
	ic := WithColumn[string]("id")
	...
	eval.Select(nc, ac, ic).Hint(keyword.Distinct).From(user).Where(predicate).GroupBy(
		eval.Group(
			nc, ac,
		),
	).Having(having).OrderBy(eval.Asc(ac), eval.Desc(nc)).Limit(20).Offset(0).Eval()
}
