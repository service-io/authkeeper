...
func (ag *autogen) RenderAuto() {
	fmt.Println(ag.option.Table)
	file := jen.NewFile("repository")

	helper.ImportPkg("system", file)
	...
	helper.WriteToFile(file, fmt.Sprintf("test/autogen/3.0/repository/%s.go", ag.option.Table), false)
}
...
func (ag *autogen) GenInterfaceRepository() jen.Code {
	...
	return jen.Line().Comment(fmt.Sprintf("i%sAutoGen 该接口自动生成, 请勿修改", ag.option.Entity)).Line().
		...
		Interface(
			...
			jen.Line().Comment("InsertByConfig Use config service to execute(insert statement)").Line().Id("InsertByConfig").Params(helper.UseSQLTx(), helper.UseIrisConfigServiceCode(helper.UseEntity(ag.option.Entity))).Bool(),
			jen.Line().Comment("UpdateByConfig Use config service to execute(update statement)").Line().Id("UpdateByConfig").Params(helper.UseSQLTx(), helper.UseIrisConfigServiceCode(helper.UseEntity(ag.option.Entity))).Bool(),
			jen.Line().Comment("DeleteByConfig Use config service to execute(delete statement)").Line().Id("DeleteByConfig").Params(helper.UseSQLTx(), helper.UseIrisConfigServiceCode(helper.UseEntity(ag.option.Entity))).Bool(),
			...
			jen.Line().Comment("Insert Matches all columns and insert into the table").Line().Id("Insert").Params(helper.UseSQLTx(), jen.Op("*").Add(helper.UseEntity(ag.option.Entity))).Int64(),
			jen.Line().Comment("InsertNonNil Matches non-nil columns and insert into the table").Line().Id("InsertNonNil").Params(helper.UseSQLTx(), jen.Op("*").Add(helper.UseEntity(ag.option.Entity))).Int64(),
			jen.Line().Comment("InsertWithFunc Matches all columns matching this function and insert into the table").Line().Id("InsertWithFunc").Params(helper.UseSQLTx(), jen.Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Func().Params(helper.UseIrisColumnCode(helper.UseEntity(ag.option.Entity)), jen.Id("any")).Bool()).Int64(),
			jen.Line().Comment("BatchInsert Batch insert all columns of table into table").Line().Id("BatchInsert").Params(helper.UseSQLTx(), jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity))).Params(jen.Index().Int64()),
			jen.Line().Comment("BatchInsertNonNil Batch insert non-nil columns(matches first element of list) of table into table").Line().Id("BatchInsertNonNil").Params(helper.UseSQLTx(), jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity))).Params(jen.Index().Int64()),
			jen.Line().Comment("BatchInsertWithFunc Batch insert all columns matching this function(matches first element of list) of table into table").Line().Id("BatchInsertWithFunc").Params(helper.UseSQLTx(), jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Func().Params(helper.UseIrisColumnCode(helper.UseEntity(ag.option.Entity)), jen.Id("any")).Bool()).Params(jen.Index().Int64()),
			jen.Line().Comment("DeleteByID Delete the related entries of id(p key)").Line().Id("DeleteByID").Params(helper.UseSQLTx(), jen.Int64()).Bool(),
			jen.Line().Comment("DeleteByIDs Delete the related entries of id(p key list)").Line().Id("DeleteByIDs").Params(helper.UseSQLTx(), jen.Op("...").Int64()).Bool(),
			jen.Line().Comment("BatchDeleteByID Batch delete the related entries of id(p key list)").Line().Id("BatchDeleteByID").Params(helper.UseSQLTx(), jen.Index().Int64()).Bool(),
			jen.Line().Comment("UpdateByID Update all columns of the related entries of id(p key)").Line().Id("UpdateByID").Params(helper.UseSQLTx(), jen.Op("*").Add(helper.UseEntity(ag.option.Entity))).Bool(),
			jen.Line().Comment("UpdateNonNilByID Update non-nil columns of the related entries of id(p key)").Line().Id("UpdateNonNilByID").Params(helper.UseSQLTx(), jen.Op("*").Add(helper.UseEntity(ag.option.Entity))).Bool(),
			jen.Line().Comment("UpdateByIDWithFunc Update all columns matching this function of the related entries of id(p key)").Line().Id("UpdateByIDWithFunc").Params(helper.UseSQLTx(), jen.Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Func().Params(helper.UseIrisColumnCode(helper.UseEntity(ag.option.Entity)), jen.Id("any")).Bool()).Bool(),
			jen.Line().Comment("BatchUpdateByIDWithFunc Batch update all columns matching this function of the related entries of id(p key)").Line().Id("BatchUpdateByIDWithFunc").Params(helper.UseSQLTx(), jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Func().Params(helper.UseIrisColumnCode(helper.UseEntity(ag.option.Entity)), jen.Id("any")).Bool()).Bool())
}
