...
import (
	"metis/model/2.0/iris/internal/constant"
...
func (e *Evaluator[T]) Select(cols ...*Column[T]) *SelectEvaluator[T] {
	...
	task := func(buffers ...*strings.Builder) []any {
		execFn := func() {
			...
			buf.WriteString(strings.Join(colLit, ", "))
		}
		switch len(buffers) {
		case 1:
			execFn()
...
func (e *SelectEvaluator[T]) GroupBy(group *Group) *SelectEvaluator[T] {
	task := func(buffers ...*strings.Builder) []any {
		for _, buffer := range buffers {
			buffer.WriteString(keyword.GroupBy.Pretty())
			buffer.WriteString(group.Literal())
		}
		return nil
	}
	e.tasks = append(e.tasks, task)
