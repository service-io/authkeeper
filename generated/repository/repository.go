...
func (ag *autogen) RenderAuto() {
	...
	file.Add(ag.GenFuncSelectXXXByYYYID())
	file.Add(ag.GenFuncSelectYYYByXXXID())
...
func (ag *autogen) GenInterfaceRepository() jen.Code {
	...
	return jen.Line().Comment(fmt.Sprintf("i%sAutoGen 该接口自动生成, 请勿修改", ag.option.Entity)).Line().
		...
		Interface(
			jen.Comment("SelectOneByConfig Use config service to execute(select statement)").Line().Id("SelectOneByConfig").Params(helper.UseCellarConfigServiceCode(helper.UseEntity(ag.option.Entity))).Params(jen.Op("*").Add(helper.UseEntity(ag.option.Entity))),
			jen.Line().Comment("SelectManyByConfig Use config service to execute(select statement)").Line().Id("SelectManyByConfig").Params(helper.UseCellarConfigServiceCode(helper.UseEntity(ag.option.Entity))).Params(jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity))),
			jen.Line().Comment("SelectPageByConfig Use config service to execute(select statement)").Line().Id("SelectPageByConfig").Params(helper.UseCellarConfigServiceCode(helper.UseEntity(ag.option.Entity))).Params(jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Int64()),
			jen.Line().Comment("InsertByConfig Use config service to execute(insert statement)").Line().Id("InsertByConfig").Params(jen.Op("*").Add(helper.UseSQLTx()), helper.UseCellarConfigServiceCode(helper.UseEntity(ag.option.Entity))).Bool(),
			jen.Line().Comment("UpdateByConfig Use config service to execute(update statement)").Line().Id("UpdateByConfig").Params(jen.Op("*").Add(helper.UseSQLTx()), helper.UseCellarConfigServiceCode(helper.UseEntity(ag.option.Entity))).Bool(),
			jen.Line().Comment("DeleteByConfig Use config service to execute(delete statement)").Line().Id("DeleteByConfig").Params(jen.Op("*").Add(helper.UseSQLTx()), helper.UseCellarConfigServiceCode(helper.UseEntity(ag.option.Entity))).Bool(),
			...
			jen.Line().Comment("InsertWithFunc Matches all columns matching this function and insert into the table").Line().Id("InsertWithFunc").Params(jen.Op("*").Add(helper.UseSQLTx()), jen.Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Func().Params(jen.Op("*").Add(helper.UseCellarColumnCode(helper.UseEntity(ag.option.Entity))), jen.Id("any")).Bool()).Int64(),
			...
			jen.Line().Comment("BatchInsertWithFunc Batch insert all columns matching this function(matches first element of list) of table into table").Line().Id("BatchInsertWithFunc").Params(jen.Op("*").Add(helper.UseSQLTx()), jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Func().Params(jen.Op("*").Add(helper.UseCellarColumnCode(helper.UseEntity(ag.option.Entity))), jen.Id("any")).Bool()).Params(jen.Index().Int64()),
			...
			jen.Line().Comment("UpdateByIDWithFunc Update all columns matching this function of the related entries of id(p key)").Line().Id("UpdateByIDWithFunc").Params(jen.Op("*").Add(helper.UseSQLTx()), jen.Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Func().Params(jen.Op("*").Add(helper.UseCellarColumnCode(helper.UseEntity(ag.option.Entity))), jen.Id("any")).Bool()).Bool(),
			jen.Line().Comment("BatchUpdateByIDWithFunc Batch update all columns matching this function of the related entries of id(p key)").Line().Id("BatchUpdateByIDWithFunc").Params(jen.Op("*").Add(helper.UseSQLTx()), jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity)), jen.Func().Params(jen.Op("*").Add(helper.UseCellarColumnCode(helper.UseEntity(ag.option.Entity))), jen.Id("any")).Bool()).Bool())
}
...
func (ag *autogen) GenFuncSelectOneByConfig() jen.Code {
	return ag.funcPrefix().Id("SelectOneByConfig").Params(helper.UseCellarNamedConfigServiceCode(helper.UseEntity(ag.option.Entity))).Params(jen.Op("*").Add(helper.UseEntity(ag.option.Entity))).
...
func (ag *autogen) GenFuncSelectManyByConfig() jen.Code {
	return ag.funcPrefix().Id("SelectManyByConfig").Params(helper.UseCellarNamedConfigServiceCode(helper.UseEntity(ag.option.Entity))).
...
func (ag *autogen) GenFuncSelectPageByConfig() jen.Code {
	return ag.funcPrefix().Id("SelectPageByConfig").Params(helper.UseCellarNamedConfigServiceCode(helper.UseEntity(ag.option.Entity))).Params(jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity)),
...
func (ag *autogen) GenFuncInsertByConfig() jen.Code {
	return ag.funcPrefix().Id("InsertByConfig").Params(jen.Id("tx").Op("*").Add(helper.UseSQLTx()), helper.UseCellarNamedConfigServiceCode(helper.UseEntity(ag.option.Entity))).
...
func (ag *autogen) GenFuncUpdateByConfig() jen.Code {
	return ag.funcPrefix().Id("UpdateByConfig").Params(jen.Id("tx").Op("*").Add(helper.UseSQLTx()), helper.UseCellarNamedConfigServiceCode(helper.UseEntity(ag.option.Entity))).
		Params(jen.Id("bool")).Block(jen.If(jen.Id("config").Op("==").Nil()).Block(jen.Return().Id("false")),
		jen.Id("evaluator").Op(":=").Id("config").Dot("Evaluator").Call(),
		jen.If(jen.Id("evaluator").Op("==").Nil()).Block(jen.Return().Id("false")),
		jen.Id("recorder").Op(":=").Add(helper.UseFetchRecorderByCtx()),
...
func (ag *autogen) GenFuncDeleteByConfig() jen.Code {
	return ag.funcPrefix().Id("DeleteByConfig").Params(jen.Id("tx").Op("*").Add(helper.UseSQLTx()), helper.UseCellarNamedConfigServiceCode(helper.UseEntity(ag.option.Entity))).
		Params(jen.Id("bool")).Block(jen.If(jen.Id("config").Op("==").Nil()).Block(jen.Return().Id("false")),
		jen.Id("evaluator").Op(":=").Id("config").Dot("Evaluator").Call(),
		jen.If(jen.Id("evaluator").Op("==").Nil()).Block(jen.Return().Id("false")),
		jen.Id("recorder").Op(":=").Add(helper.UseFetchRecorderByCtx()),
	...
	return ag.funcPrefix().Id("SelectByID").Params(jen.Id("id").Int64()).Params(jen.Op("*").Add(helper.UseEntity(ag.option.Entity))).
		Block(jen.Id("config").Op(":=").Add(helper.UseEntity(fmt.Sprintf("New%s", ag.option.Entity))).Call(),
			jen.Id("config").Dot("Configure").Call(
				jen.Func().Params(jen.Id("eval").Op("*").Add(helper.UseCellarEvaluatorCode(helper.UseEntity(ag.option.Entity)))).
					Block(jen.Id("eval").Dot("Select").Call(jen.Id("config").Dot("Asterisk").Call().Op("...")).
						...
						Add(helper.InferCode(helper.HasColumn(helper.TtKey, ag.option.Cols), jen.Dot("And").Call(helper.UseNSPredicate("config"))))).
						Dot("Eval").Call())),
...
func (ag *autogen) GenFuncBatchSelectByID() jen.Code {
	return ag.funcPrefix().Id("BatchSelectByID").Params(jen.Id("ids").Index().Int64()).
		...
		Block(jen.Id("values").Op(":=").Id("make").Call(jen.Index().Id("any"),
			...
			jen.Id("config").Dot("Configure").Call(jen.Func().Params(jen.Id("eval").Op("*").Add(helper.UseCellarEvaluatorCode(helper.UseEntity(ag.option.Entity)))).
				Block(jen.Id("eval").Dot("Select").Call(jen.Id("config").Dot("Asterisk").Call().Op("...")).
					Dot("From").Call(jen.Id("config").Dot("Table").Call()).
			...
			code := ag.funcPrefix(jen.Line(), jen.Line()).Id(fmt.Sprintf("SelectBy%sID", cn)).Params(jen.Id("id").Int64()).Params(jen.Index().Op("*").Add(helper.UseEntity(ag.option.Entity))).
				Block(jen.Id("config").Op(":=").Add(helper.UseEntity(fmt.Sprintf("New%s", ag.option.Entity))).Call(),
					jen.Id("config").Dot("Configure").Call(jen.Func().Params(jen.Id("eval").Op("*").Add(helper.UseCellarEvaluatorCode(helper.UseEntity(ag.option.Entity)))).
						Block(jen.Id("eval").Dot("Select").Call(jen.Id("config").Dot("Asterisk").Call().Op("...")).
							Dot("From").Call(jen.Id("config").Dot("Table").Call()).
							Dot("Where").Call(jen.Id("config").Dot(fmt.Sprintf("%sIdCol", cn)).Call().Dot("EQ").Call(jen.Id("id")).
							Add(helper.InferCode(helper.HasColumn(helper.TtKey, ag.option.Cols) && !(col.ColumnName == helper.TtKey), jen.Dot("And").Call(helper.UseNSPredicate("config"))))).Dot("Eval").Call())),
					jen.Return().Id("ag").Dot("SelectManyByConfig").Call(jen.Id("config")))
				...
				Block(jen.Id("config").Op(":=").Add(helper.UseEntity(fmt.Sprintf("New%s", ag.option.Entity))).Call(),
					jen.Id("config").Dot("Configure").Call(jen.Func().Params(jen.Id("eval").Op("*").Add(helper.UseCellarEvaluatorCode(helper.UseEntity(ag.option.Entity)))).
						Block(jen.Id("eval").Dot("Select").Call(jen.Id("config").Dot("Asterisk").Call().Op("...")).
							...
							Dot("Like").Call(jen.Id("val")).Add(helper.InferCode(helper.HasColumn(helper.TtKey, ag.option.Cols), jen.Dot("And").Call(helper.UseNSPredicate("config"))))).Dot("Eval").Call())),
					jen.Return().Id("ag").Dot("SelectManyByConfig").Call(jen.Id("config")))
			strCodes = append(strCodes, code)
		}
	}
...
func (ag *autogen) GenFuncSelectAllWithPage() jen.Code {
	return ag.funcPrefix().Id("SelectAllWithPage").Params(jen.Id("page").Int64(),
		...
		jen.Id("config").Dot("Configure").Call(jen.Func().Params(jen.Id("eval").Op("*").Add(helper.UseCellarEvaluatorCode(helper.UseEntity(ag.option.Entity)))).
			Block(jen.Id("eval").Dot("Select").Call(jen.Id("config").Dot("Asterisk").Call().Op("...")).
				Dot("From").Call(jen.Id("config").Dot("Table").Call()).
				Dot("Where").Call(jen.Add(helper.InferCode(helper.HasColumn(helper.TtKey, ag.option.Cols), helper.UseNSPredicate("config")))).
				Dot("Page").Call(jen.Id("size"),
...
func (ag *autogen) GenFuncInsert() jen.Code {
	return ag.funcPrefix().Id("Insert").Params(jen.Id("tx").Op("*").Add(helper.UseSQLTx()),
		...
		jen.Func().Params(jen.Op("*").Add(helper.UseCellarColumnCode(helper.UseEntity(ag.option.Entity))),
...
func (ag *autogen) GenFuncInsertNonNil() jen.Code {
	return ag.funcPrefix().Id("InsertNonNil").Params(jen.Id("tx").Op("*").Add(helper.UseSQLTx()),
		jen.Id(ag.option.Variable).Op("*").Add(helper.UseEntity(ag.option.Entity))).Int64().Block(jen.Return().Id("ag").Dot("InsertWithFunc").Call(jen.Id("tx"),
		jen.Id(ag.option.Variable),
		jen.Func().Params(jen.Id("c").Op("*").Add(helper.UseCellarColumnCode(helper.UseEntity(ag.option.Entity))),
...
func (ag *autogen) GenFuncInsertWithFunc() jen.Code {
	return ag.funcPrefix().Id("InsertWithFunc").Params(jen.Id("tx").Op("*").Add(helper.UseSQLTx()),
		...
		jen.Id("fn").Func().Params(jen.Op("*").Add(helper.UseCellarColumnCode(helper.UseEntity(ag.option.Entity))),
...
func (ag *autogen) GenFuncBatchDeleteByID() jen.Code {
	deleteCode := jen.Id("eval").Dot("Delete").Call().Dot("From").Call(jen.Id("config").Dot("Table").Call()).
		Dot("Where").Call(jen.Id("config").Dot("IDCol").Call().Dot("IN").Call(jen.Id("values").Op("..."))).Dot("Eval").Call()
	// eval.Update(config.Table()).Set(config.DeletedCol(), 1).Set(config.ModifyAtCol(), time.Now()).Set(config.ModifyByCol(), security.GetAccountID(ag.ctx)).Where(config.IDCol().IN(values...)).Eval()
	updateCode := jen.Id("eval").Dot("Update").Call(jen.Id("config").Dot("Table").Call()).
		Dot("Set").Call(jen.Id("config").Dot(fmt.Sprintf("%sCol", strcase.ToCamel(helper.DKey))).Call(), jen.Lit(1)).
		Dot("Set").Call(jen.Id("config").Dot(fmt.Sprintf("%sCol", strcase.ToCamel(helper.MaKey))).Call(), jen.Add(helper.UseTime("Now")).Call()).
		Dot("Set").Call(jen.Id("config").Dot(fmt.Sprintf("%sCol", strcase.ToCamel(helper.MbKey))).Call(), helper.UseGetAccountID()).
		Dot("Where").Call(jen.Id("config").Dot("IDCol").Call().Dot("IN").Call(jen.Id("values").Op("...")).
		Add(helper.InferCode(helper.HasColumn(helper.TtKey, ag.option.Cols), jen.Dot("And").Call(helper.UseNSPredicate("config"))))).Dot("Eval").Call()
	var code = deleteCode
	if helper.HasColumn(helper.DKey, ag.option.Cols) {
		code = updateCode
	}
...
func (ag *autogen) GenFuncSelectShip(XXX, YYY *helper.Option) jen.Code {
	...
	existTenantId := helper.HasColumn(helper.TtKey, ag.option.Cols) || helper.HasColumn(helper.TtKey, XXX.Cols) || helper.HasColumn(helper.TtKey, YYY.Cols)
	return ag.funcPrefix().Id(fmt.Sprintf("Select%sBy%sID", XXX.GetShipKey(), YYY.GetShipKey())).
		...
		Block(
			...
			jen.Id("leftConfig").Dot("Configure").Call(
				jen.Func().Params(jen.Id("eval").Op("*").Add(helper.UseCellarEvaluatorCode(helper.UseEntity(XXX.Entity)))).
					Block(jen.Id("leftTable").Op(":=").Id("leftConfig").Dot("Table").Call(),
						...
						jen.Id("eval").Dot("Select").Call(jen.Id("leftConfig").Dot("Asterisk").Call(jen.Id("leftTable").Dot("Decorate")).Op("...")).
							...
							Dot("Where").Call(
							jen.Id("shipConfig").Dot(fmt.Sprintf("%sIdCol", YYY.GetShipKey())).Call().Dot("Decorate").
								Call(jen.Id("shipTable").Dot("Decorate")).Dot("EQ").Call(jen.Id(fmt.Sprintf("%sID", strcase.ToLowerCamel(YYY.GetShipKey())))).
								Add(helper.InferCode(existTenantId, jen.Dot("And").Call(
									helper.InferCode(helper.HasColumn(helper.TtKey, ag.option.Cols), helper.UseNSPredicate("shipConfig")),
									helper.InferCode(helper.HasColumn(helper.TtKey, XXX.Cols), helper.UseNSPredicate("leftConfig")),
									helper.InferCode(helper.HasColumn(helper.TtKey, YYY.Cols), helper.UseNSPredicate("rightConfig")),
								))),
						).
